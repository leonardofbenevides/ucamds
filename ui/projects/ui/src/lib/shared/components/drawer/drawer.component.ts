  ...
  template: `
    <ng-template #panel>
      <z-drawer-panel
        ...
        [class]="class()"
        (closeRequested)="requestClose('swipe')"
      >
        <ng-content />
      </z-drawer-panel>
    </ng-template>
...
export class ZardDrawerComponent extends ZardDrawerHost {
  ...
  /** PATCH UCAM: by which path the close was asked for. */
  readonly zCloseRequested = output<ZardDrawerCloseReason>();
  ...
  requestClose(reason: ZardDrawerCloseReason = 'programmatic'): void {
    if (!this.zDismissible()) {
      return;
    }
    // PATCH UCAM: o motivo sai ANTES de fechar, para quem escuta poder
    // decidir com o painel ainda montado.
    this.zCloseRequested.emit(reason);
  ...
  private open(): void {
    ...
    overlayRef.backdropClick().subscribe(() => this.requestClose('scrim'));
    overlayRef
      .keydownEvents()
...
export class ZardDrawerCloseDirective {
  ...
  protected onClick(): void {
    this.drawer?.requestClose('button');
  }
}
