...
@Component({
  ...
  template: `
    <button
      ...
      [attr.aria-label]="zTriggerId() ? null : triggerAriaLabel()"
...
export class ZardSelectComponent implements ControlValueAccessor, OnDestroy {
  ...
  /**
   * ADICIONADO PELA UCAM (ADR-006). Id do BOTÃO gatilho, para que um
   * <label for> externo o alcance — o <button> é elemento rotulável. Sem
   * isto o único nome acessível vinha de triggerAriaLabel, que é o VALOR
   * escolhido: o campo se anunciava "Colegiado Direito" em vez de "Setor".
   *
   * CONFLITA com um futuro `zard-cli add select`: a reinstalação apaga isto.
   */
  readonly zTriggerId = input<string | null>(null);
  ...
  protected readonly triggerClasses = computed(() =>
    mergeClasses(selectTriggerVariants({}), this.zMultiple() && 'h-auto min-h-9 py-1'),
  );
