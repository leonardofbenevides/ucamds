// TRADUZIDO PARA pt-BR em 06/09/2026 (ADR-006: a ZardUI é matéria-prima
// copiada, não dependência — editar é legítimo). São TRÊS strings de leitor
// de tela cravadas na base: as instruções de teclado e as duas mensagens da
// região aria-live. Não dava para sobrescrever de fora: são conteúdo do
  ...
  template: `
    <div [class]="classes()">
      <div id="command-instructions" class="sr-only">
        Use as setas para percorrer, Enter para escolher e Esc para fechar.
      </div>
...
export class ZardCommandComponent implements ControlValueAccessor, ZardCommand {
  ...
  protected readonly statusMessage = computed(() => {
    ...
    if (!filteredCount) {
      return `Nada encontrado para "${searchTerm}"`;
    }

    return `${filteredCount} resultado${filteredCount === 1 ? '' : 's'} para "${searchTerm}"`;
  });
